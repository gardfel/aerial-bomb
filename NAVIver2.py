...
class Rocket(object):
    ...
    def navigation(self, *args):
        ...
        a_m = 1.  # коэффициент быстроты реакции ракеты на маневр цели (от 1 до бесконечности)
        ...
        d_delta_max = 10 * kk.pi / 180  # * 20 * 4  # скорость отклонения рулей
        ...
        while (rast_min >= 5) and (t <= 250) and ((rast_krit - rast_min) <= 200) and (self.y >= 0):
            # flag_v.append(650)
            # self.delt = kk.asin()
        ...
        if fin_ch <= 0:
            # print(sum(fxx_t), sum(fyy_t), sum(da_ball))
            krizzz = krit_maxi(self.m0, sum(fyy_t), sum(fxx_t), sum(da_ball), self.paramu)
            if krizzz >= 1000:
                krizzz = 0.1
            return krizzz
        else:
            ...
            for i in range(len(tt)):
                LL.append(kk.sqrt(xx[i] ** 2 + yy[i] ** 2))
            plt.plot(tt, LL)
            plt.show()"""
...
def krit_maxi(m_0_w, Sfy, Sfx, dealf, parame):
    ...
    pipi = [[5776, 4505, 6056], [4581, 3706, 4899], [3783, 3169, 4130], [2776, 2132, 2855], [3066, 2332, 3159], [8559, 6684, 9607]]
    k_paramz = 0
    return (m_0_w / m_0_w_0) ** (0) * (Sfy / pipi[parame][1]) ** (2) * (Sfx / pipi[parame][0]) ** (-4) * (dealf / pipi[parame][2]) ** (-1)


"""pipi_01 = [5776, 4505, 6056]  # v320, x3000, y3500
pipi_02 = [4581, 3706, 4899]  # v220, x3000, y3500
pipi_03 =[3783, 3169, 4130]  # v120, x3000, y3500
pipi_04 = [2776, 2132, 2855]  # v-320, x3000, y3500
pipi_05 = [3066, 2332, 3159]   # v-320, x4000, y3500
pipi_06 = [8559, 6684, 9607]  # v320, x5000, y3500"""
...
"""
# params_kr = np.array([0.098, 0.009172, 0.10462, 0, 0.015])
# params_op = np.array([0.12576, 0.00385, 0.034, 0, 0.330])

# params_korp = ([0.0974, 0.050, 0.160])
"""params_kr = np.array([0.11026240986850604, 0.008016503210508974, 0.0969384848389594, 0.0, 0.011894709601854021])
params_op = np.array([0.0720518452736283, 0.0023326516626669316, 0.03237462765607622, 0.0, 0.3310667916006867])"""
targ = Target(40000, 0, 20)
